...
class ChannelCore(RequestCore):
    ...
    async def async_next(self):
        ...
        if self.data.status_code != 200:
            raise YouTubeRequestError(f'Invalid status code {self.data.status_code} for channel next request')
    ...
    async def async_create(self):
        ...
        self.data = await self.asyncPostRequest()
        if self.data.status_code != 200:
            raise YouTubeRequestError(f'Invalid status code {self.data.status_code} for channel request')
        self.parse_response()
