...
class Comments:
    ...
    async def init(self) -> None:
        if self.__comments is None:
            self.__comments = CommentsCore(self.playlistLink, timeout=self.timeout)
    ...
    @staticmethod
    async def get(videoLink: str, timeout: int = None) -> Union[dict, str, None]:
        pc = CommentsCore(videoLink, timeout=timeout)
...
class Transcript:
    @staticmethod
    async def get(videoLink: str, params: str = None, timeout: int = None):
        transcript_core = TranscriptCore(videoLink, params, timeout=timeout)
